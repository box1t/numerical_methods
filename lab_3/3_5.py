...
def rungeError(values, hs, p):
    if len(values) < 2 or len(hs) < 2:
        raise ValueError("Для Рунге требуется как минимум два значения интеграла и два шага.")
    if hs[0] <= 0 or hs[1] <= 0:
    ...
    denominator = k ** p - 1
    if abs(denominator) < sys.float_info.epsilon:
        raise ValueError(f"Знаменатель в Рунге близок к нулю (k^p - 1), k={k}, p={p}.")
...
if integral is not None:
    ...
    if len(hs) >= 2 and len(integralsRectangles) >= 2 and len(integralsTrapezoids) >= 2 and len(integralsSimpson) >= 2:
        print("--- Уточненные значения по Рунге: R_h ≈ (I_h - I_{h/k}) / (1 - k^p)")
        try:
            ...
            print(f"Метод прямоугольников")
            print(f"\tАпостериорная оценка погрешности по Рунге (для шага {hs[1]}): {runge_error_rectangles_h1}")
            print(f"\tУточненное значение: {integralRectanglesRunge}")
            print(f"\tАбсолютная погрешность (от истинного): {errorRectanglesRunge}")
        except ValueError as e:
            print(f"Ошибка при применении правила Рунге для прямоугольников: {e}", file=sys.stderr)
        ...
        try:
            ...
            print(f"Метод трапеций")
            print(f"\tАпостериорная оценка погрешности по Рунге (для шага {hs[1]}): {runge_error_trapezoids_h1}")
            print(f"\tУточненное значение: {integralTrapezoidsRunge}")
            print(f"\tАбсолютная погрешность (от истинного): {errorTrapezoidsRunge}")
        except ValueError as e:
            print(f"Ошибка при применении правила Рунге для трапеций: {e}", file=sys.stderr)
        ...
        try:
            ...
            print(f"Метод Симпсона")
            print(f"\tАпостериорная оценка погрешности по Рунге (для шага {hs[1]}): {runge_error_simpson_h1}")
            print(f"\tУточненное значение: {integralSimpsonRunge}")
            print(f"\tАбсолютная погрешность (от истинного): {errorSimpsonRunge}")
        except ValueError as e:
            print(f"Ошибка при применении правила Рунге для Симпсона: {e}", file=sys.stderr)