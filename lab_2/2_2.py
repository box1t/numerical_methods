...
def get_LU_decomposition(A):
    ...
    for k in range(n):
        ...
        for i in range(k + 1, n):
            if abs(LU[i][k]) > abs(LU[pivot_row][k]):
                pivot_row = i
...
def solve_system(LU, swaps, b):
    ...
    for i in range(n - 1, -1, -1):
        ...
        if np.isclose(LU[i][i], 0):
            return None

        x[i] = (z[i] - sum_ux) / LU[i][i]
...
def newton(x0, eps, interval):
    ...
    while (True):
        iter += 1
        jac_f = fDer(xPrev)
        (LU, swaps) = get_LU_decomposition(jac_f)

        if LU is None:
             print(f"ПРЕДУПРЕЖДЕНИЕ: Сингулярная матрица Якоби на итерации {iter}. Метод прерван.")
             return xPrev, iter

        delta_x = solve_system(LU, swaps, -f(xPrev))

        if delta_x is None:
            print(f"ПРЕДУПРЕЖДЕНИЕ: Не удалось решить систему для Delta x на итерации {iter}. Метод прерван.")
            return xPrev, iter
        ...
        if not is_within_interval(xCur, interval):
             ...
             return None, iter # Возвращаем None, так как корень не найден в области
...
def simpleIterations(x0, interval, eps):
    xPrev = np.copy(x0)
    iter = 0
    ...
    max_sin_x2 = max(abs(np.sin(x2_min)), abs(np.sin(x2_max)))
    k_start = int(np.floor((x2_min - np.pi/2) / np.pi))
    k_end = int(np.ceil((x2_max - np.pi/2) / np.pi))
    for k in range(k_start, k_end + 1):
        critical_point = np.pi/2 + k * np.pi
        if x2_min <= critical_point <= x2_max:
            max_sin_x2 = 1.0
            break

    q_sin = max_sin_x2 / 3.0
    q_exp = np.exp(x1_max) / 3.0
    q_calculated = max(q_sin, q_exp)

    print(f"Расчетное значение q (max ||phi'(x)||inf на интервале [{x1_min}, {x1_max}] x [{x2_min}, {x2_max}]): {q_calculated:.6f}")

    if q_calculated >= 1:
        print("ПРЕДУПРЕЖДЕНИЕ: Условие сходимости ||phi'(x)|| <= q < 1 не выполнено в данной области.")
        print("Сходимость метода простых итераций в этой области не гарантируется.")
    ...
    while (True):
        ...
        if not is_within_interval(xCur, interval):
             ...
             return None, iter # Возвращаем None, так как корень не найден в области
        if q_calculated < 1:
            error_apriori = q_calculated / (1 - q_calculated) * np.linalg.norm(xCur - xPrev, np.inf)
...
if not is_within_interval(x0, interval):
     print("ОШИБКА: Начальное приближение", x0, "находится вне указанного интервала [", x1_min, ",", x1_max, "] x [", x2_min, ",", x2_max, "].")
     sys.exit()
...
# Метод Ньютона
newtonAns, newton_iter = newton(x0, eps, interval) # Передаем интервал
...
# Метод простых итераций
simpleIterationsAns, simple_iter = simpleIterations(x0, interval, eps) # Передаем интервал
...
y_vals_eq1 = np.linspace(plot_y_min, plot_y_max, 400)
x_vals_eq1 = np.cos(y_vals_eq1) / 3.0

x_vals_eq2 = np.linspace(plot_x_min, plot_x_max, 400)
y_vals_eq2 = np.exp(x_vals_eq2) / 3.0
...
plt.plot([x1_min, x1_max, x1_max, x1_min, x1_min], [x2_min, x2_min, x2_max, x2_max, x2_min], color='gray', linestyle='--', label='Область поиска')
...
plt.savefig('graph_system_solution_with_region_check.png')
